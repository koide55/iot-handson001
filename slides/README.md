# IoT Hands-On Slides

このディレクトリには、`Raspberry Pi Pico 2 W with headers` を使った IoT と Cortex-M33 アセンブラの講義資料をまとめています。

## 資料一覧

| 回 | 内容 | ファイル |
|---|---|---|
| 1 | IoT 演習の導入、GPIO、アナログ入力、I2C センサ、Wi-Fi、Ambient | [lecture01_iot_security_pico2w.md](lecture01_iot_security_pico2w.md) |
| 2 | Cortex-M33 アセンブラの実機ハンズオン本文 | [lecture02_assembly_handson_restructured.md](lecture02_assembly_handson_restructured.md) |
| 2.5 | 演習前に読む Cortex-M33 アセンブラ超入門 | [lecture02a_cortex_m33_assembly_primer.md](lecture02a_cortex_m33_assembly_primer.md) |
| 3 | Cortex-M33 アセンブラの命令解説と演習整理 | [lecture03_cortex_m33_assembly.md](lecture03_cortex_m33_assembly.md) |
| 4 | Cortex-M33 と Hazard3 の比較発展資料 | [lecture04_cortex_m33_vs_hazard3.md](lecture04_cortex_m33_vs_hazard3.md) |
| 5 | ローカル Python 受信サーバと Pico 送信コードの仕様書 | [lecture05_mtd_local_post_spec.md](lecture05_mtd_local_post_spec.md) |
| 5.5 | 固定パス切り換えの設計議論 | [lecture05a_path_rotation_design_discussion.md](lecture05a_path_rotation_design_discussion.md) |

## 読み方のおすすめ

IoT 演習から入る場合は次の順番を勧めます。

1. [lecture01_iot_security_pico2w.md](lecture01_iot_security_pico2w.md)
2. [lecture02a_cortex_m33_assembly_primer.md](lecture02a_cortex_m33_assembly_primer.md)
3. [lecture02_assembly_handson_restructured.md](lecture02_assembly_handson_restructured.md)
4. [lecture03_cortex_m33_assembly.md](lecture03_cortex_m33_assembly.md)

## 関連プロジェクト

アセンブラ演習で使うプロジェクトは `../projects/` にあります。

- 完成版: [../projects/pico2w-cortexm33-asm-complete/README.md](../projects/pico2w-cortexm33-asm-complete/README.md)
- 穴埋め版: [../projects/pico2w-cortexm33-asm-worksheet/README.md](../projects/pico2w-cortexm33-asm-worksheet/README.md)

## English

English index:

- [README_en.md](README_en.md)
- [lecture01_iot_security_pico2w_en.md](lecture01_iot_security_pico2w_en.md)
- [lecture02_assembly_handson_restructured_en.md](lecture02_assembly_handson_restructured_en.md)
- [lecture02a_cortex_m33_assembly_primer_en.md](lecture02a_cortex_m33_assembly_primer_en.md)
- [lecture03_cortex_m33_assembly_en.md](lecture03_cortex_m33_assembly_en.md)
- [lecture04_cortex_m33_vs_hazard3_en.md](lecture04_cortex_m33_vs_hazard3_en.md)
- [lecture05_mtd_local_post_spec_en.md](lecture05_mtd_local_post_spec_en.md)
