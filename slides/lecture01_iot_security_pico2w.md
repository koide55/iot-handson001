...
- 抵抗器 10MΩ を 1 本（8 章のアナログ入力で使用）
- タクトスイッチ 1 個
- BMP280 + AHT20 一体型センサモジュール 1 個
- インターネット接続できる PC
- 2.4GHz 帯の Wi-Fi
...
- 全員が `GP15` と `GND` の位置を指で示せるか
- 少なくとも 1 回、全員の LED 点滅を目視確認したか
...
`INPUT_PULLUP` を使うと、タクトスイッチを押していないときは `HIGH`、押したときは `LOW` になります。

### 7.2.1 なぜプルアップが必要か

マイコンの入力ピンは、そのままでは「何にもつながっていない」状態になることがあります。この状態を **フローティング** と呼びます。
...
が安定せず、周囲のノイズの影響で値がふらついてしまいます。

そこで、入力ピンを普段は **弱く `HIGH` 側へ引っ張っておく** ために、プルアップを使います。今回の演習では、外付け抵抗ではなく `INPUT_PULLUP` による **内部プルアップ** を使います。

### 7.2.2 なぜ OFF で `HIGH`、ON で `LOW` になるのか
...
さらに、`INPUT_PULLUP` を指定すると、マイコン内部で `GP14` が弱く `3.3V` 側へ引き上げられます。

その結果、状態は次のようになります。

#### タクトスイッチを押していないとき

- `GP14` は内部プルアップによって `3.3V` 側へ引かれる
- `GND` にはまだつながっていない
- そのため入力は `HIGH`

#### タクトスイッチを押したとき

- `GP14` と `GND` がスイッチを通してつながる
- `GP14` は `0V` 側へ落ちる
- そのため入力は `LOW`
...
### 7.3 タクトスイッチの 4 端子について

今回使うタクトスイッチは 4 本足ですが、4 本が全部ばらばらに独立しているわけではありません。

分かりやすく言うと、**向かい合う 2 本ずつが 1 組** になっています。

- 押していないとき:
  - 片側の 2 本は、もともとつながっている
  - 反対側の 2 本も、もともとつながっている
  - ただし、2 つの組どうしはまだつながっていない
- 押したとき:
  - 2 つの組どうしもつながる
  - 結果として 4 本すべてが導通する
...
そのため、ブレッドボードでは **片側の組を `GP14`、反対側の組を `GND`** へつなぐようにしてください。同じ組の 2 本に配線してしまうと、押していなくても常につながった状態になります。
...
- タクトスイッチの同じ組の 2 本に配線してしまう
- `INPUT_PULLUP` を入れ忘れる
- 押したときが `LOW` であることを忘れる
- `GP14` ではなく別のピンに挿している

### 7.4.4 講師チェックポイント

- 1 人の受講者に「押したとき `HIGH` か `LOW` か」を口頭で答えてもらう
- 代表者の回路で、タクトスイッチの向きを全員に確認させる
...
授業で使う LED としては、**赤色の 5mm LED を推奨** します。
...
- 手で覆う、室内光、スマートフォンのライトで ADC 値に差が出る
...
数値そのものは環境で変わります。大事なのは、条件を変えると値も変わることです。

### 8.7.3 よくある失敗

- `10MΩ` ではなく小さい抵抗を使っている
- LED の向きが発光時と同じになっている
- `A0` と `GND` の位置を取り違えている

### 8.7.4 講師チェックポイント

- 少なくとも 1 台で「手で覆う」と「ライトを当てる」で値が変わることを全員に見せる
- 数値の絶対値ではなく、差を見る演習であることを再確認する
...
ただし、BMP280 は基板や配線条件によって `0x77` 以外のアドレスで見える場合があります。もし I2C スキャナで `0x77` ではない BMP280 アドレスが見えた場合は、**以後のプログラム中でもそのアドレスへ読み替えてください**。
...
となっている箇所は、実際に見えたアドレスに合わせて変更する必要があります。

### 9.5 センサ単体の確認

ライブラリ付属のサンプルで確認しても構いませんが、Pico 2 W では I2C ピンを明示したほうが分かりやすいので、ここでは最初から最小コードを示します。今回の基板では、BMP280 のアドレスは `0x77` 前提で進めます。もしスキャナ結果が別アドレスだった場合は、ここもその値に読み替えてください。
...
- シリアルモニタに温度、湿度、気圧が 2 秒ごとに表示される

期待されるシリアル出力例:

```text
BMP temp [C]: 24.31
BMP pressure [hPa]: 1008.42
AHT temp [C]: 24.87
AHT humidity [%]: 51.23
```
...
- `SDA` と `SCL` を逆につなぐ
- `bmp.begin(0x77)` のアドレスを実機に合わせていない
- `Wire.setSDA(4)` と `Wire.setSCL(5)` を忘れる
- `VDD` と `GND` を逆にする

### 9.5.4 講師チェックポイント

- まず I2C スキャナ結果を全員で確認したか
- BMP280 のアドレスが `0x77` 以外の受講者がいないかを口頭で確認したか
...
- 5GHz のみの SSID を使っている
- SSID やパスワードの入力ミス
- 企業ネットワークで接続制限がある

### 10.6 講師チェックポイント

- IP アドレス表示まで進んだ受講者が半数以上いるか
- 企業ゲストネットワークなど代替手段が必要かを早めに判断したか
...
- Ambient のチャネルに少なくとも 1 件以上データが表示される
- `d1`, `d2`, `d3` に対応するグラフが作れる

### 11.7 よくある失敗

- `CHANNEL_ID` や `WRITE_KEY` の転記ミス
- 送信間隔が短すぎる
- HTTP の応答を確認せずに進めてしまう
...
タクトスイッチを押したときだけ LED が点灯するようにしてください。
...
- 起動直後に LED が 3 回点滅する
- その後は、タクトスイッチを押している間だけ LED が点灯する
