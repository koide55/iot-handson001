...
- LED が約 0.3 秒ごとに点灯、消灯を繰り返す

### 8.3 よくある失敗

- `GP15` と `GND` の取り違え
- LED の向きが逆
- CPU Architecture が正しくないまま作業を進める

### 8.4 講師チェックポイント

- 全員が C 版 L チカ成功、または失敗原因を特定した状態で先へ進む
...
- `orrs` ではなく別の命令を書いてしまう
- `nop` を消したあとに行全体を崩してしまう

### 講師チェックポイント

- `r0` がアドレス、`r1` が読み出した値だと全員が理解しているか
- 1 人以上に「なぜ `1` を使うのか」を口頭で説明してもらう
...
- `bit を落とす` と `bit を反転する` を混同する
- `bics` を使わず、意図しない値操作をしてしまう

### 講師チェックポイント

- `ON` と `OFF` の違いを、ビット操作として説明できるか
- 全員がここまでで `.S` の編集と保存を経験したか
...
- 少なくとも 1 人に `0 ^ 1 = 1` と `1 ^ 1 = 0` を口頭で説明してもらう
- 受講者全体の半数以上がここで自力成功しているか確認する
...
- 押していないとき: LED 状態は変化しない
- 押している間: LED が短い間隔で反転を繰り返す（エッジ検出はしていないため）
...
- `INPUT_PULLUP` の論理を忘れて `0` と `1` を逆に考える
- タクトスイッチの配線ミスで常時 ON になる

### 講師チェックポイント

- `押したときが LOW` であることを全員が理解しているか
- 1 台の回路を前に出して、押下時だけ反転する様子を全員で確認したか
...
- 開始ビット番号と長さを逆に書く
- `bit 4 から 2 bit` の意味を取り違える

### 講師チェックポイント

- 1 人に `0xB0` の 2 進表現を書いてもらい、bit 4 と bit 5 を一緒に確認する
...
- `r1` の下位 2 bit だけを使うことを忘れる
- 埋め込み位置を bit 4 ではなく別位置にしてしまう

### 講師チェックポイント

- `UBFX` と `BFI` が「抜き出す」「埋め込む」の対になっていると説明できるか
...
```text
RBIT source      : 0x12345678
RBIT result      : 0x1E6A2C48
```
...
- `RBIT` は bit 順、`REV` は byte 順だと明確に言い分けているか
...
```text
REV source       : 0x12345678
REV result       : 0x78563412
```
...
- `0x12345678 -> 0x78563412` を全員が追えるか確認する
...
- `0x12345678` の最上位側を見て数えることを、その場で全員に再確認する
