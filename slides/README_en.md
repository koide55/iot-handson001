# IoT Hands-On Slides

This directory contains the lecture materials for the `Raspberry Pi Pico 2 W with headers` IoT hands-on and the Cortex-M33 assembly hands-on.

## Slide List

| Session | Topic | File |
|---|---|---|
| 1 | IoT hands-on introduction, GPIO, analog input, I2C sensors, Wi-Fi, and Ambient | [lecture01_iot_security_pico2w.md](lecture01_iot_security_pico2w.md) |
| 2 | Main Cortex-M33 assembly hands-on guide | [lecture02_assembly_handson_restructured.md](lecture02_assembly_handson_restructured.md) |
| 2.5 | Gentle Cortex-M33 assembly primer to read before the hands-on | [lecture02a_cortex_m33_assembly_primer.md](lecture02a_cortex_m33_assembly_primer.md) |
| 3 | Cortex-M33 assembly explanation and exercise overview | [lecture03_cortex_m33_assembly.md](lecture03_cortex_m33_assembly.md) |
| 4 | Advanced comparison of Cortex-M33 and Hazard3 | [lecture04_cortex_m33_vs_hazard3.md](lecture04_cortex_m33_vs_hazard3.md) |
| 5 | Specification for a local Python receiver and Pico sender | [lecture05_mtd_local_post_spec.md](lecture05_mtd_local_post_spec.md) |

## English Versions

| Session | Topic | File |
|---|---|---|
| 1 | IoT hands-on introduction, GPIO, analog input, I2C sensors, Wi-Fi, and Ambient | [lecture01_iot_security_pico2w_en.md](lecture01_iot_security_pico2w_en.md) |
| 2 | Main Cortex-M33 assembly hands-on guide | [lecture02_assembly_handson_restructured_en.md](lecture02_assembly_handson_restructured_en.md) |
| 2.5 | Gentle Cortex-M33 assembly primer to read before the hands-on | [lecture02a_cortex_m33_assembly_primer_en.md](lecture02a_cortex_m33_assembly_primer_en.md) |
| 3 | Cortex-M33 assembly explanation and exercise overview | [lecture03_cortex_m33_assembly_en.md](lecture03_cortex_m33_assembly_en.md) |
| 4 | Advanced comparison of Cortex-M33 and Hazard3 | [lecture04_cortex_m33_vs_hazard3_en.md](lecture04_cortex_m33_vs_hazard3_en.md) |
| 5 | Specification for a local Python receiver and Pico sender | [lecture05_mtd_local_post_spec_en.md](lecture05_mtd_local_post_spec_en.md) |

## Recommended Reading Order

If learners start from the IoT hands-on, the following order works well.

1. [lecture01_iot_security_pico2w.md](lecture01_iot_security_pico2w.md)
2. [lecture02a_cortex_m33_assembly_primer.md](lecture02a_cortex_m33_assembly_primer.md)
3. [lecture02_assembly_handson_restructured.md](lecture02_assembly_handson_restructured.md)
4. [lecture03_cortex_m33_assembly.md](lecture03_cortex_m33_assembly.md)

## Related Projects

The assembly hands-on projects are stored under `../projects/`.

- Complete version: [../projects/pico2w-cortexm33-asm-complete/README.md](../projects/pico2w-cortexm33-asm-complete/README.md)
- Worksheet version: [../projects/pico2w-cortexm33-asm-worksheet/README.md](../projects/pico2w-cortexm33-asm-worksheet/README.md)

## Japanese

Japanese index:

- [README.md](README.md)
